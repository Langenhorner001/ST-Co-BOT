...
_PAYPAL_AU_TOKEN = os.environ.get("PAYPAL_AU_TOKEN", "").strip()
if not _PAYPAL_AU_TOKEN:
    import logging as _glog
    _glog.getLogger(__name__).warning(
        "[GATET] PAYPAL_AU_TOKEN env var not set — PayPal gateway will fail. "
        "Set this in your environment secrets."
    )
...
def ahmed(ccx: str) -> str:
    ...
    try:
        # Step 1: Fetch donation page to prime cookies + extract form values dynamically
        page_resp = sess.get(_DONATE_URL, headers=_BASE_HEADERS, timeout=_REQUEST_TIMEOUT)
        page_html = page_resp.text

        # Dynamic extraction — fallback to hardcoded if regex fails
        _m_prefix = re.search(r'name="give-form-id-prefix"\s+value="([^"]+)"', page_html)
        _m_id     = re.search(r'name="give-form-id"\s+value="([^"]+)"', page_html)
        _m_hash   = re.search(r'name="give-form-hash"\s+value="([^"]+)"', page_html)

        form_id_prefix = _m_prefix.group(1) if _m_prefix else _FORM_ID_PREFIX
        form_id        = _m_id.group(1)     if _m_id     else _FORM_ID
        form_hash      = _m_hash.group(1)   if _m_hash   else _FORM_HASH
