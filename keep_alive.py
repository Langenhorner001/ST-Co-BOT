...
# ── Per-IP rate limiter for /api/stripe (S2 fix) ────────────────────────
# Limits each IP to _RL_MAX requests per _RL_WINDOW seconds
import collections
_rl_lock   = __import__('threading').Lock()
_rl_store  = collections.defaultdict(list)
_RL_MAX    = 10    # max requests per window
...
def _rate_limited(ip: str) -> bool:
    ...
    with _rl_lock:
        ...
        _rl_store[ip] = [t for t in reqs if now - t < _RL_WINDOW]
...
def api_stripe():
    # ── Per-IP rate limiting (S2 fix) ─────────────────────────────
    client_ip = flask_request.headers.get('X-Forwarded-For', flask_request.remote_addr or '').split(',')[0].strip()
    if _rate_limited(client_ip):
        return 'ERROR | Rate limit exceeded. Max 10 requests/minute per IP.', 429
...
def api_stripe_json():
    ...
    client_ip = flask_request.headers.get('X-Forwarded-For', flask_request.remote_addr or '').split(',')[0].strip()
    if _rate_limited(client_ip):
        return jsonify({'error': 'Rate limit exceeded. Max 10 requests/minute per IP.'}), 429
